...
export class Board {
  private _walledDisc: Disc[][]

  constructor(private _discs: Disc[][]) {
    this._walledDisc = this.wallDiscs()
  }
  ...
  private listFlipPoints(move: Move): Point[] {
    ...
    const checkFlipPoints = (xMove: number, yMove: number) => {
      ...
      while (isOppositeDisc(move.disc, this._walledDisc[cursorY][cursorX])) {
        // 番兵を考慮して−１する
        flipCandidate.push(new Point(cursorX - 1, cursorY - 1))
        cursorX += xMove
        cursorY += yMove
        // 次の手が同じ色の石なら、ひっくり返す石が確定
        if (move.disc === this._walledDisc[cursorY][cursorX]) {
          flipPoinsts.push(...flipCandidate)
          break
        }
      }
    }
...
export const InitalBoard = new Board(INITIAL_DISCS)
