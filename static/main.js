...
function showWarningMessage(previousDisc, nextDisc, winnerDisc) {
  const message = warningMesage(previousDisc, nextDisc, winnerDisc)
...
function warningMesage(previousDisc, nextDisc, winnerDisc) {
  if( nextDisc !== null){
...
function showNextDiscMessage(nextDisc) {
  if ( nextDisc) {
    nextDiscMessageElement.innerHTML = `次は${discToString(nextDisc)}の番です`
  } else {
